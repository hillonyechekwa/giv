...
export function DataTable<TData, TValue>({ columns, data, title, onRowClick }: DataTableProps<TData, TValue>) {
  ...
  // Handle row click and prevent navigation when clicking on checkboxes
  const handleRowClick = (event: React.MouseEvent<HTMLTableRowElement>, row: Row<TData>) => {
    ...
    const isCheckboxClick = (event.target as HTMLElement).closest('[role="checkbox"]') !== null

    // Only navigate if it wasn't a checkbox click and we have an onRowClick handler
