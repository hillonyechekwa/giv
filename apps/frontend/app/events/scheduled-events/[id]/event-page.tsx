...
const ScheduledEventTemplatePage: React.FC<Props> = ({ id }) => {
    const router = useRouter()

    const { data: scheduledEvent, isLoading } = useQuery<EventType>({
        queryKey: ['scheduledEvent'],
        queryFn: async () => {
    ...
    console.log("draft event", scheduledEvent);

    // Early return if loading or if scheduledEvent is not yet available
    if (isLoading || !scheduledEvent) {
        return <Loader />;
    }
    ...
    const winners = scheduledEvent.winners || [];
    const prizes = scheduledEvent.prizes || []
    const clicks = scheduledEvent.clicks || 0;
    const scans = scheduledEvent.scans || 0;
                        ...
                        <TabsContent value="outline" className="mt-0">
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <Input defaultValue={scheduledEvent.name || ""} className="border rounded-lg p-4" />
                                </div>

                                <div>
                                    <h3 className="text-base font-medium mb-2">Description</h3>
                                    <Textarea
                                        defaultValue={scheduledEvent.description || ""}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <div className="relative">
                                            <Input
                                                type="text"
                                                defaultValue={safeFormat(scheduledEvent.date, "MMMM d, yyyy")}
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <Input
                                                type="text"
                                                defaultValue={safeFormat(scheduledEvent.eventStartTime, "h:mm a")}
                                                className="border rounded-lg p-4 pr-10"
                                            />
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <Input
                                                type="text"
                                                defaultValue={safeDuration(scheduledEvent.eventStartTime, scheduledEvent.eventEndTime)}
                                                className="border rounded-lg p-4 pr-10"
                                            />
                                ...
                                <p className="text-sm text-gray-500">
                                    ...
                                    {safeFormat(scheduledEvent.date, "MMMM d, yyyy")} from {safeFormat(scheduledEvent.eventStartTime, "h:mm a")} to {safeFormat(scheduledEvent.eventEndTime, "h:mm a")}
                                </p>
                            </div>
...
export default ScheduledEventTemplatePage;